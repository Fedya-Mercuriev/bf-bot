    ...
    async requestTime(ctx) {
        ...
        this._hasDateAndShipping(orderDate, shipping)
            .then(async() => {
                ...
                const returnedMessage = await ctx.replyWithHTML(`Введите желаемое время в формате "ЧЧ:ММ".\n⚠Нам потребуется <b>${estimatedTime / 60000} мин.</b>, чтобы сделать букет${additionalMessage}.\n🗓 Сегодня мы работаем с ${start}:00 до ${finish}:00`);
                this.messages = {
                    messageType: 'intro',
